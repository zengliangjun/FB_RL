...
# Pre-defined configs
##
from envs.isaac.robots.g1 import G1_CYLINDER_CFG
    ...
    while simulation_app.is_running():
        ...
        if not file_saved:
            ...
            critic = [
                        ...
                        robot.data.root_pos_w[:, 2].unsqueeze(-1),
                        ...
                        robot.data.joint_stiffness / robot.data.default_joint_stiffness,
                        robot.data.joint_damping / robot.data.default_joint_damping,
            ...
            policy = torch.cat(policy, dim = -1)
            critic = torch.cat(critic, dim = -1)

            log['observations'].append(policy.cpu().numpy().copy())
            log['privileges'].append(critic.cpu().numpy().copy())
