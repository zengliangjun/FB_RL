...
from cpr.updaters import actorupdater
...
class ActorUpdater(actorupdater.ActorUpdater):
    ...
    def _calcute_disc(self, state: torch.Tensor, action: torch.Tensor, z_policy: torch.Tensor):
        # 计算判别器奖励损失
        Qs_discriminator = self.calcute.critic(state, action, z_policy)  # num_parallel x batch x (1 or n_bins)

        if isinstance(Qs_discriminator, Union[tuple, list]) and len(Qs_discriminator) == 2:
            Qs_discriminator = torch.min(*Qs_discriminator)
        else:
            if hasattr(self.calcute, 'get_targets_uncertainty'):
                Qs_discriminator = self.calcute.get_targets_uncertainty(Qs_discriminator, self.config.pessimism_penalty)

        return Qs_discriminator
