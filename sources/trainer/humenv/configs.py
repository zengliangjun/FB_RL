...
@dataclasses.dataclass
class  CriticConfig(net_configs.SimpleCriticConfig):
    num_parallel: int = 2
...
#### Model
@dataclasses.dataclass
class  FBModelConfig(models_configs.FBCprModelConfig):
...
@dataclasses.dataclass
class  ActorUpdaterConfig(cpr_updaters_configs.ActorConfig):
...
#### Agent
@dataclasses.dataclass
class FBAgentConfig(agents_configs.FBCPrAgentConfig):
...
@dataclasses.dataclass
class  MotionBufferConfig(buffers_configs.MotionBufferConfig):
...
### Env
@dataclasses.dataclass
class  HumEnvConfig(envs_configs.HumenvConfig):
    pass

@dataclasses.dataclass
class TrainerConfig(trainers.BaseTrainerConfig):
    _target_: str = "trainer.humenv.trainer:FBTrainer"  # 目标类路径
    ...
    one_step_collect_iters: int = 10
    one_step_update_iters: int = 50
    ...
    def __post_init__(self):
        ...
        self._eval_names_ = ["eval_reward", "eval_tracking"]
